...
class itemsController extends Controller
{
    ...
    public function updateSkinDatabase($bitApikey,$googleAuthCode){
    ...
    public function getBitskins(){
        $json = json_decode(file_get_contents('https://bitskins.com/api/v1/get_all_item_prices/?api_key=aa1e0e34-3f74-42bf-848a-b2e64ef31f66&code=477653&app_id=730.json'),true);
        $allitems=$json['prices'];
        foreach ($allitems as $itemDetails){
            $itemfullname=$itemDetails["market_hash_name"];
            $gunFullName=explode(" | ",$itemDetails["market_hash_name"]);
            ...
            if($skinName!=null && $gunName!=null){
                ...
                //var_dump($skinParent);
                 if(isset($skinParent) && $skinParent->image==null){
                     $skinParent->image=$itemDetails["icon_url"];
                     $skinParent->save();
                 }
                if($skinParent!=null){
                    $item=new item_wear();
                    $item->market_hash_name=$itemDetails["market_hash_name"];
                    $item->image=$itemDetails["icon_url"];
                    $item->skin_id=$skinParent->id;
                    $item->steamPrice=$itemDetails["price"];
                    $item->bitskinsPrice=$itemDetails["price"];
                    $item->steamLink="https://steamcommunity.com/market/listings/730/".$itemDetails["market_hash_name"];
                    $item->save();
                }
